...
contract NFinTech is IERC721 {
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata data) public {
        ...
        //     == IERC721TokenReceiver.onERC721Received.selector, "unsafe recipient"
        // );
        if (to.balance == 0) {
        bytes4 onERC721ReceivedSelector = IERC721TokenReceiver(to).onERC721Received(msg.sender, from, tokenId,data);
        require(onERC721ReceivedSelector == IERC721TokenReceiver.onERC721Received.selector, "Unsafe recipient");
        }
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public {
        ...
        // );
        if (to.balance == 0) {
        bytes4 onERC721ReceivedSelector = IERC721TokenReceiver(to).onERC721Received(msg.sender, from, tokenId,"");
        require(onERC721ReceivedSelector == IERC721TokenReceiver.onERC721Received.selector, "Unsafe recipient");
        }
    }
}